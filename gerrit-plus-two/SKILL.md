...
2. **评审Comment** — 以独立评论请求添加格式为 `领域SE_{MODULE}_代码review_OK_{SE_NAME}` 的SE评审comment（**若已存在相同格式的 Comment 则跳过**）
...
**去重逻辑**：添加 Comment 前会先检查是否已存在相同格式的 `领域SE_{MODULE}_代码review_OK_{SE_NAME}` 的评论，若已存在则跳过添加步骤。
