...
def check_se_comment_exists(base_url, change_id, revision, se_comment):
    """检查指定 Change 的评论中是否已存在相同格式的 SE 评审 Comment。

    返回 True 表示已存在，无需重复添加。
    """
    encoded_id = urllib.parse.quote(change_id, safe="")
    url = f"{base_url}/a/changes/{encoded_id}/comments"
    try:
        data = gerrit_request(url, base_url=base_url)
        # 遍历所有 patchSet 和其下的评论
        patch_sets = data or {}
        for ps_key, comments in patch_sets.items():
...
示例:
  ...
  python3 gerrit_plus_two.py --change 879887 --dry-run
    ...
    if args.dry_run:
        ...
        # 先检查是否已存在相同格式的 SE 评审 Comment
        if check_se_comment_exists(base_url, args.change, args.revision, se_comment):
            print(f"  2b. SE 评审 Comment 已存在（{se_comment}），跳过添加。")
            comment_ok = True
        else:
